...
/// Channel rate targeted for wide-band FM. Wide enough for the ±75 kHz deviation
/// limit and (in a later phase) the 57 kHz RDS subcarrier in the recovered
/// baseband, which needs ≥ 120 kHz of discriminator output bandwidth.
pub const WFM_TARGET_HZ: f64 = 250_000.0;
...
///
/// The factor is rounded, never exact, so the *actual* channel rate is
/// [`channel_rate`] rather than the target — reported as such, in keeping with
/// the app's rule of showing measured values instead of intended ones.
pub fn decimation_factor(sample_rate: f64, target_hz: f64) -> usize {
    if !(sample_rate > 0.0) || !(target_hz > 0.0) {
        return 1;
    }
    (sample_rate / target_hz).round().max(1.0) as usize
}
...
/// 250 kHz channel rate that is ±125 kHz, comfortably clear of the 75 kHz WFM
/// limit. Yields `len − 1` outputs: the missing first sample is precisely the
/// block-splice guard, since the previous block's last phase is not usable.
...
/// tuning error as modulation and inflate the deviation figure.
...
mod tests {
    ...
    #[test]
    fn decimation_factor_rounds_and_floors_at_one() {
        assert_eq!(decimation_factor(2_000_000.0, 250_000.0), 8);
        assert_eq!(decimation_factor(2_400_000.0, 250_000.0), 10); // 9.6 → 10
    ...
    fn channel_rate_is_derived_not_assumed() {
        // 2.4 Msps cannot hit 250 kHz exactly — the honest rate is 240 kHz.
        let d = decimation_factor(2_400_000.0, WFM_TARGET_HZ);
        assert!((channel_rate(2_400_000.0, d) - 240_000.0).abs() < 1e-6);
    }
