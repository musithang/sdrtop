...
pub fn spacers_to_drop(total: usize, blank_idx: &[usize], avail: usize) -> Vec<usize> {
    ...
    if excess >= blank_idx.len() {
        return blank_idx.to_vec();
    }
    (0..excess)
        .map(|k| blank_idx[k * blank_idx.len() / excess])
        .collect()
}
...
pub fn collapse_spacers(lines: &mut Vec<Line<'_>>, avail: usize) {
    ...
    let blank_idx: Vec<usize> = lines
        .iter()
        .enumerate()
        .filter(|(_, l)| l.spans.iter().all(|s| s.content.trim().is_empty()))
        .map(|(i, _)| i)
        .collect();
    let drop: std::collections::HashSet<usize> = spacers_to_drop(lines.len(), &blank_idx, avail)
        .into_iter()
        .collect();
    if drop.is_empty() {
        return;
...
mod tests {
    ...
    fn section_fills_full_inner_width() {
        ...
        let hinted = super::section(
            "DEADLINE BUDGET",
            "\u{250a} = \u{00b1}600 \u{00b5}s",
            44,
            &t,
        );
    ...
    fn field_leading_space_and_left_padded() {
        ...
        assert_eq!(
            ...
            12,
            "1 lead space + 11-wide label"
        );
        assert!(s.content.starts_with(" Rate"));
        assert!(
            s.content.ends_with("       "),
            "short label is right-padded with spaces"
        );
    }
    ...
    fn spacers_to_drop_keeps_all_when_it_fits() {
        ...
        assert!(
            spacers_to_drop(20, &blanks, 20).is_empty(),
            "exact fit drops nothing"
        );
        assert!(
            spacers_to_drop(18, &blanks, 20).is_empty(),
            "room to spare drops nothing"
        );
    }
    ...
    fn spacers_to_drop_indices_distinct_and_sized() {
        ...
        for excess in 1..13 {
            ...
            assert_eq!(
                drop.len(),
                unique.len(),
                "excess={excess}: no repeated index"
            );
            assert_eq!(
                drop.len(),
                excess,
                "excess={excess}: drops exactly `excess`"
            );
        }
    }
    ...
    fn wrap_keeps_a_verdict_headline_whole() {
        ...
        assert_eq!(
            rows,
            vec!["\u{26a0} WFM CARRIER \u{2014} ADJACENT", "SPLATTER"]
        );
    }
