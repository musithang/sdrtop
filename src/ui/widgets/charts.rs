...
pub fn gain_bar_colored(
    ...
    for x in 0..n {
        ...
        let col = if filled {
            lerp_color(lo, hi, x as f64 / denom)
        } else {
            empty_col
        };
...
pub fn null_meter(
    ...
    let frac = if full_scale > 0.0 {
        (value / full_scale).clamp(-1.0, 1.0)
    } else {
        0.0
    };
    let needle =
...
pub fn mini_braille_line(data: &[f32], width: usize) -> String {
    ...
    for col in 0..width {
        ...
        if ll >= 0 {
            ...
            for l in ll.min(prev)..=ll.max(prev) {
                bits |= LEFT[l as usize];
            }
        }
        ...
        if rl >= 0 {
            ...
            for l in rl.min(prev)..=rl.max(prev) {
                bits |= RIGHT[l as usize];
            }
        }
...
pub fn braille_profile(data: &[f32], cols: usize, rows: usize) -> Vec<String> {
    ...
    for (p, &v) in data.iter().take(cols * 2).enumerate() {
        ...
        let height = (((v - lo) / range) * total_levels as f32)
            .round()
            .clamp(0.0, total_levels as f32) as i32;
...
mod tests {
    ...
    fn braille_profile_geometry() {
        ...
        for r in &rows {
            for c in r.chars() {
                assert!((0x2800..=0x28FF).contains(&(c as u32)), "{c:?}");
            }
        }
    }
    ...
    fn braille_profile_fills_upward_from_the_floor() {
        ...
        assert_eq!(
            cell(0, 0),
            '\u{2800}',
            "the lowest samples reach nowhere near the top row"
        );
        assert_ne!(cell(1, 0), '\u{2800}', "but they do light the bottom row");
        assert_eq!(
            cell(1, 3),
            '\u{28FF}',
            "the highest samples fill the bottom row through"
        );
    ...
    fn braille_profile_resolves_a_spike_more_rows_can_show() {
        ...
        assert_eq!(
            level(3),
            2,
            "three rows place the pilot two rows up from the floor"
        );
    }
    ...
    fn mini_braille_line_rising_ramp_is_braille() {
        ...
        for c in mini_braille_line(&data, 4).chars() {
            assert!(
                c as u32 >= 0x2800 && c as u32 <= 0x28FF,
                "non-braille char: {c:?}"
            );
        }
    }
    ...
    fn mini_braille_line_sparse_history_fills_from_left() {
        ...
        let s: Vec<char> = mini_braille_line(&[0.0, 1.0, 2.0, 3.0], 6)
            .chars()
            .collect();
        assert!(
            s[0] as u32 > 0x2800,
            "left end should carry dots, got {:?}",
            s[0]
        );
    ...
    fn bipolar_strip_zero_lights_centered_baseline() {
        ...
        assert_eq!(
            rows[0].chars().next().unwrap() as u32,
            0x2800,
            "top row should be blank"
        );
        assert_ne!(
            rows[1].chars().next().unwrap() as u32,
            0x2800,
            "baseline row should carry a dot"
        );
    }
    ...
    fn bipolar_strip_sign_fills_up_and_down() {
        ...
        assert_ne!(
            up[0].chars().next().unwrap() as u32,
            0x2800,
            "positive reaches the top row"
        );
        assert_ne!(
            down[1].chars().next().unwrap() as u32,
            0x2800,
            "negative reaches the bottom row"
        );
    ...
    fn null_meter_needle_deflects_with_sign() {
        ...
        let pos: String = null_meter(0.8, 1.0, 17, col, dim)
            .iter()
            .map(|s| s.content.as_ref())
            .collect();
        let neg: String = null_meter(-0.8, 1.0, 17, col, dim)
            .iter()
            .map(|s| s.content.as_ref())
            .collect();
        let center = 9; // includes ◄
        assert!(
            pos.chars().position(|c| c == '●').unwrap() > center,
            "positive deflects right"
        );
        assert!(
            neg.chars().position(|c| c == '●').unwrap() < center,
            "negative deflects left"
        );
    }
