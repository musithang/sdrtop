    ...
    vec![
        fit(
            vec![
                ...
                Span::styled(
                    format!("{:.3} MHz", carrier_hz / 1e6),
                    Style::default()
                        .fg(tint.carrier)
                        .add_modifier(Modifier::BOLD),
                ),
                Span::styled(
                    format!(" \u{00b7} {:.1} dBFS", r.carrier_dbfs),
        ...
        fit(
            vec![
                ...
                Span::styled(
                    format!("\u{00b7} {:.1} dBFS", r.image_dbfs),
        ...
        fit(
            vec![
                ...
                Span::styled(
                    format!("\u{00b7} {:.1} dBFS", r.dc_dbfs),
                    Style::default().fg(tint.dc),
                ),
...
pub(super) fn caption(theme: &crate::Theme) -> Line<'static> {
    Line::from(vec![
        Span::raw(" "),
        Span::styled(
            "image mirrors the carrier about the LO",
...
fn fit(mut spans: Vec<Span<'static>>, iw: usize) -> Line<'static> {
    while spans.len() > ESSENTIAL_SPANS && width(&spans) > iw {
        spans.pop();
    }
    Line::from(spans)
}
...
mod tests {
    ...
    fn the_essential_spans_are_never_cut() {
        ...
        assert_eq!(
            l.spans.len(),
            ESSENTIAL_SPANS,
            "the value must survive any width"
        );
