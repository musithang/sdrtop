            ...
            for (c, &h) in heights.iter().enumerate() {
                ...
                if c == peak_lo || c == peak_hi {
                    spans.push(Span::styled(
                        "\u{254e}".to_string(),
                        Style::default().fg(peak_col),
                    ));
                } else if c == sig_lo || c == sig_hi {
                    spans.push(Span::styled(
                        "\u{2506}".to_string(),
                        Style::default().fg(sig_col),
                    ));
                } else {
    ...
    if maxc == 0 {
        ...
        out.push(Line::from(vec![
            ...
            Span::styled(
                format!(
                    " peak \u{00b1}{:.0}%",
                    (levels.peak_frac * 100.0).min(100.0)
                ),
                lbl,
            ),
            ...
            Span::styled(
                format!(
                    " 1\u{03c3} \u{00b1}{:.0}%",
                    (levels.sigma_frac * 100.0).min(100.0)
                ),
                lbl,
            ),
        ]));
...
mod tests {
    ...
    fn caliper_is_symmetric_about_centre() {
        ...
        assert!(
            ((centre - lo as f64) - (hi as f64 - centre)).abs() <= 1.0,
    ...
    fn per_axis_sigma_is_magnitude_rms_over_sqrt2() {
        ...
        assert!(
            a < mag,
            "per-axis σ must be narrower than the magnitude RMS"
        );
    }
    ...
    fn per_axis_sigma_inside_peak_for_typical_signal() {
        ...
        assert!(
            a_sigma < a_peak,
            "1σ {a_sigma} should be inside peak {a_peak}"
        );
    }
}
