...
pub(super) fn lines(
    out: &mut Vec<Line<'static>>, state: &SdrMetrics, stale: bool, iw: usize, theme: &crate::Theme,
) {
    ...
    out.push(metric("Crest / PAPR", vec![Span::styled(format!("{:.1} dB", load.crest_db), val(theme))], theme));
}
