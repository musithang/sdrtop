...
use crate::ui::widgets::timing_fmt::{fmt_us, ppm_span, quality_color};
...
impl Panel for TimingDiagnosticsPanel {
    ...
    fn render(
        ...
        lines.push(Line::from(if stale || t.cb_period_us == 0 {
            ...
            vec![
                field("Period"),
                Span::styled(fmt_us(t.cb_period_us), val),
        ...
        lines.push(Line::from(if stale {
            ...
            vec![
                field("Jitter"),
                Span::styled(format!("\u{00b1}{} \u{00b5}s rms", t.cb_jitter_us), val),
            ]
        }));
        lines.push(Line::from(if stale {
            vec![field(""), dash()]
        ...
        for (i, (name, v)) in bars.iter().enumerate() {
            ...
            let bar_w = iw
                .saturating_sub(1 + 4 + 1 + 1 + value_str.chars().count())
                .max(6);
        ...
        lines.push(Line::from(if stale {
            ...
            let col = if t.late_callbacks * 20 > t.late_window {
                theme.status_crit
            } else {
                theme.status_warn
            };
        ...
        lines.push(Line::from(
            if stale || state.radio.actual_sample_rate == 0 {
                vec![
                    field("Rate"),
                    Span::styled(format!("{cfg_msps:.3} MHz"), val),
                    Span::raw("  "),
                    dash(),
                ]
            } else {
                ...
                vec![
                    field("Rate"),
                    Span::styled(format!("{cfg_msps:.3} \u{2192} {act:.3} MHz"), val),
                ]
            },
        ));
        lines.push(Line::from(
        ...
        lines.push(Line::from(if stale {
            ...
            vec![
                field("Throughput"),
                Span::styled(format!("{:.1} MB/s", t.throughput_mean_mbps), val),
                Span::styled(format!("   \u{03c3} {:.2}", t.throughput_std_mbps), dim),
            ]
        }));
        let thist: Vec<f64> = state
            .radio
            .throughput_history
            .iter()
            .map(|&v| v as f64)
        ...
        if stale {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(" reset peak  ", lbl),
                Span::styled("[C]", key_style),
                Span::styled(" clear counters", lbl),
            ]));
        }
