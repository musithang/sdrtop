...
pub(super) fn seam(
    f: &mut Frame, area: Rect, plot: Rect,
    window: Window, status: &Status, border: Color, theme: &crate::Theme,
) {
    // The rule, aligned to the plot rather than the panel, so a tick sits over
    // the column it names.
    let ruler = freq_scale_spans(window.left_hz, window.bw, plot.width as usize,
                                 border, theme.value, '\u{2500}');
    f.render_widget(
        Paragraph::new(Line::from(ruler)),
        Rect { x: plot.x, y: area.y, width: plot.width, height: 1 },
    );
    ...
    if tags.is_empty() || area.height < 2 { return; }
    ...
    f.render_widget(
        ...
        Rect { x: (area.x + area.width).saturating_sub(2 + w),
               y: area.y + area.height - 1, width: w, height: 1 },
    );
}
...
fn status_tags(status: &Status, theme: &crate::Theme) -> Vec<Span<'static>> {
    ...
    if status.paused {
        tags.push(Span::styled("\u{23F8} ", Style::default().fg(theme.status_warn)));
    } else if status.stale {
        tags.push(Span::styled("STALE ", Style::default().fg(theme.stale)));
    }
    if status.stride > 1 {
        tags.push(Span::styled(format!("\u{00D7}{} ", status.stride), Style::default().fg(theme.label)));
    }
    if status.scroll > 0 {
        tags.push(Span::styled(format!("\u{2191}{} ", status.scroll), Style::default().fg(theme.value_hi)));
    }
    tags
}
...
mod tests {
    ...
    fn the_status_cap_says_only_what_is_true() {
        ...
        assert!(status_tags(&quiet, &t).is_empty(), "nothing to report, no cap");

        let busy = Status { paused: true, stale: true, stride: 4, scroll: 12 };
        assert_eq!(text(&status_tags(&busy, &t)), "\u{23F8} \u{00D7}4 \u{2191}12 ",
