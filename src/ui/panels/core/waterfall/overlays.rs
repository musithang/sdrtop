...
pub(super) fn time_axis(
    ...
    rows: &VecDeque<(Instant, Arc<Vec<f32>>)>, skip_data: usize, theme: &crate::Theme,
) {
    ...
    for k in 1..=4 {
        ...
        let Some((ts, _)) = rows.get(data_idx) else { continue };
        ...
        f.render_widget(
            ...
            Rect { x: area.x + area.width - lw, y: area.y + r as u16, width: lw, height: 1 },
        );
    }
}
