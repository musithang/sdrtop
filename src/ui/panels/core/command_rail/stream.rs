...
pub(super) fn lines(state: &SdrMetrics, active: bool, theme: &crate::Theme) -> Vec<Line<'static>> {
    let value = |s: String| Span::styled(
        s, Style::default().fg(if active { theme.value } else { theme.label }));
    let row = |name: &str, v: String| Line::from(vec![
        Span::raw(" "), label_cell(name, theme), value(v),
    ]);
    vec![
        ...
        Line::raw(""),
        row("USB", fmt_mb(if active { state.radio.current_throughput_bps } else { 0 })),
    ]
}
