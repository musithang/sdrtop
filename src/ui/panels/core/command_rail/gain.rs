...
pub(super) fn lines(state: &SdrMetrics, active: bool, observer: bool,
    ...
    let mut total_spans = vec![
        Span::raw(" "), label_cell("TOTAL", theme),
        Span::styled(format!("{total} dB"), Style::default().fg(val_col)),
    ];
    if active {
        let headroom = (-state.signal.adc_peak_dbfs).max(0.0);
        total_spans.push(Span::styled("  \u{00b7}  ".to_string(), Style::default().fg(theme.border_dim)));
...
fn gain_row(label: Span<'static>, gain: u32, max: u32, lo: Color, hi: Color,
            bar_w: usize, active: bool, val_col: Color, theme: &crate::Theme) -> Line<'static> {
    let bar: Vec<Span<'static>> = if active {
        ...
        vec![Span::styled(filled, Style::default().fg(theme.label)),
