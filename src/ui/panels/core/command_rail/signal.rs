...
fn metric_block(m: Metric<'_>, iw: usize, theme: &crate::Theme) -> Line<'static> {
    let Metric { label, unit, value, color: value_color, history, arrow } = m;
...
pub(super) fn lines(state: &SdrMetrics, stale: bool, active: bool,
    ...
    let mut out = vec![
        metric_block(Metric {
            ...
            color: snr_color(snr, theme), history: &sig.snr_history,
        ...
        metric_block(Metric {
            ...
            color: theme.value, history: &sig.pwr_history,
        ...
        metric_block(Metric {
            ...
            color: theme.value, history: &sig.nf_history,
        ...
        metric_block(Metric {
            ...
            color: sat_color(sat, theme), history: &sig.sat_history,
    ...
    out.push(match clip_alert(sig.last_clip_at, now) {
        Some((since, fresh)) => {
            let mut style = Style::default()
                .fg(if fresh { theme.status_crit } else { theme.stale });
