...
pub(super) fn frequency(f: &mut Frame, area: Rect, canvas_width: u16, view: &SpectrumView,
                       border: Color, theme: &crate::Theme) {
    let spans = freq_scale_spans(view.left_hz, view.bw, canvas_width as usize,
                                 border, theme.value, ' ');
...
pub(super) fn tuning(f: &mut Frame, area: Rect, freq_hz: u64, step_hz: u64,
                     cursor: Option<(f64, f32)>, theme: &crate::Theme) {
    ...
    let arrow = Style::default().fg(theme.border_accent).add_modifier(Modifier::BOLD);
    f.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("\u{2500}".repeat(left_arm), Style::default().fg(theme.border_dim)),
            ...
            Span::styled("\u{2500}".repeat(right_arm), Style::default().fg(theme.border_dim)),
...
pub(super) fn db_gutter(f: &mut Frame, area: Rect, y_min: f32, y_max: f32,
                        border: Color, theme: &crate::Theme) {
    ...
    let lines: Vec<Line> = row_label.iter().map(|label| {
