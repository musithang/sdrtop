...
That is the whole of it. Everything else in this document is either a
consequence of that sentence or an illustration of what happens without it.
...
*Everything above, stated flatly. The story is what makes them stick. This is
what they say.*

### ONE

> **No world is charted from orbit.**

A radio gets a native backend after it has been on this desk. Powered. Tuned to
something genuinely transmitting. Watched by a person who was in a position to
notice something subtly wrong and permitted, at that moment, to be disappointed.
Not after the datasheet was read closely. After it was **held**.

A datasheet cannot be asked a follow-up question, and everything that has ever
mattered was in the follow-up question. This rule is expensive, and the expense
is why the supported list is short and moves at the speed of a hobby budget.

*The tide gauge. The mountain.*

### TWO

> **What cannot be asked is refused, never invented.**

Where sdrtop cannot work something out, that reading says so and stops. No
plausible default. No interpolation from a similar device. No last known value
carried quietly forward.

A missing number is an inconvenience. A crash is embarrassing and completely
honest. A wrong number that looks right is neither, and it is the only thing this
whole document is trying to prevent.

*The Great Agreement.*

### THREE

> **A payload is not testimony.**

Printing `51 bytes` is a courier's job. The question is *why 51*: at what
spreading factor, across what bandwidth, how far above the noise, how much of the
margin the preamble spent, how many symbols were repaired on the way in, and
whether the decoder was confident or merely lucky.

A signal is a physical event, and the payload is very nearly the least
interesting thing about it. Every digital demodulator on the roadmap is held to
this or it does not ship.

*The Courier Called As An Expert.*

### FOUR

> **All testimony is dated.**

Every lab panel marks itself **[STALE]** the moment RX stops. A number on a
screen is a claim about *now*, and the instant that stops being true while the
number stays put, the screen is lying in the most persuasive way available to any
interface: by not changing.

*The lighthouse keeper.*

### FIVE

> **One witness, one account.**

No reading may say two different things in two parts of the screen. One scale for
one quantity, everywhere it appears, and every label names what was actually
measured so the label and the number cannot drift apart.

*The Great Agreement, in miniature.*

### SIX

> **Exhibits are checked against the events, never against each other.**

Documentation is held to these rules exactly as the code is. Each fact lives in
one place and the other pages point at it. The same thing explained two ways in
two files is not thoroughness, it is two exhibits that have started drifting and
have not been caught. Where the docs are unsure they say so in the same font as
everything else.
...
> **A safeguard that cannot fail is an ornament.**

If a check cannot be made to fail on demand, it is not a check. A reference
generated from the code is trusted; a reference typed out by hand is not, however
carefully it was typed. A verification step whose failure path continues anyway
is worse than none at all, because now there is a tick mark in front of the thing
nobody checked.
...
> **The court declines what it holds no instrument for.**

There is no audio. There is no transmit, on hardware perfectly capable of it. No
mouse input is handled anywhere. The dBm figures on the RF bench are modelled and
relative and the panel says so on its own face.

Every one of those is a ruling, and a ruling is not a missing feature. An honest
estimate, labelled an estimate, is a measurement. The same number with the label
removed is perjury with better typesetting.

*Eleven requests to rule on beauty.*

### NINE

> **The second opinion has to come from somebody who does not care how it comes
> out.**

An instrument that refuses to invent will visibly not know things, all over its
own interface, permanently. Those gaps are not failures to paper over. They are
the invitation, and the most valuable ones sit in front of radios nobody here has
ever held.

*The visitor, and her rather short plaque.*

### TEN

> **A record nobody finishes has recorded nothing.**

Accuracy is necessary and has never once been sufficient, which is why there is a
planet in this document. The constraint that makes that safe is absolute: **the
story lives in the frame and never touches the reading.** Be as funny as you like
about the institution, the history, the desk or the author. Never about what a
number means.

The gradient runs the other way as the reader nears the machinery. This page is
the most story-bearing thing in the repository; `troubleshooting.md` is close to
humourless on purpose, because nobody arrives there in a good mood. Plain is not
the same as dry, and not one page here is permitted to be dry.

*The debt this document owes to a particular Englishman with a towel is
considerable, and is hereby entered into the record.*

---

# THE MARKED EXCEPTION

Rule One is not satisfied for any radio reached through **SoapySDR**. Airspy, the
SDRplay RSP line, PlutoSDR, LimeSDR, bladeRF, USRP, and whatever sits at the far
end of SoapyRemote. None has been on this desk. The backend was written from the
API headers.

The evidence is inadmissible, and it is admitted anyway, under the doctrine
above, with an instruction that does not get quieter further down the page:
...
The conditions, all already met:

- **No device table exists in the source.** Frequency range, sample rates, gain
  range, AGC, baseband filter, sample format and how many of its bits carry
  meaning: every one is asked of the driver at runtime.
- **Rule Two is untouched.** Where the driver cannot answer, the reading is
  absent. The RF bench's noise figure and linearity card does not appear at all
  on a SoapySDR device, because SoapySDR does not publish a front end. Inventing
  it would have taken an evening and would have been the worst thing in this
  program.
- **Another party's claim is entered as another party's claim.** `SoapyHackRF`
  announces 0 to 7.25 GHz where the datasheet says 1 MHz to 6 GHz. sdrtop repeats
  the driver's number and the log names the driver that said it. It does not
  silently correct evidence it cannot verify, and it does not launder it either.
- **The marking is everywhere the evidence is.** On the badge, in the status
  block, and in a hardware table that is not flattering and is not going to be
  improved.

Every one of those radios stays grey until somebody who owns one stands it on a
...
*Attached at the request of the applicant, who asked to say this himself.*

I broke my own rule. I want that written down by me, not paraphrased by a court I
invented.

The rule was good. Hardware support lands only after physical testing, no
guessing from datasheets, and I meant it. It was also why, every single week,
somebody asked whether this would talk to their Airspy and the honest answer was
no, and would stay no, because buying one of everything is not a plan, it is a
wish with a shipping address.

Here is what actually decided it. An instrument gets better in exactly one way:
somebody runs it on real hardware and comes back to tell me what it got wrong.
Under the rule as written, the only people able to do that were HackRF and
RTL-SDR owners. That is a very thin crew to carry a program whose whole ambition
is to be the most careful thing in your terminal.
...
making sure it could not spread than I spent writing the backend.
...
edge. I would rather have a rule with one marked scar on it than a rule nobody
has ever had a reason to test.

If you own one of those radios: **you are the visitor with your own gauge.** You
are the only thing that has ever ended a Great Agreement. An issue saying "it
works, here is what it reported" is worth as much to me as a crash report, and
arguably more, because a crash tells me where it broke and a working report tells
me where it held.

You will not be detained for eleven days. I have neither the authority nor the
room.

---

If this program ever tells you something it does not actually know, that is not a
bug report. That is perjury, and it outranks every open issue and every feature
anybody is waiting for. Say so, loudly. You will be believed.
